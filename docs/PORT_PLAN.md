...
- **Chunked large-volume behavior** — Dask's overlap/stitching semantics differ from the hand-rolled `Chunk`/`ObjectStitcher` system; validate object counts/boundaries match on a volume too large for RAM before trusting it in production. Now has a design of its own: [`docs/LARGE_IMAGES.md`](LARGE_IMAGES.md) covers the memory budget, the tile plan, the reconciliation rules for objects a tile boundary cuts in half, and a phase-by-phase strategy for every protocol step (under review).
